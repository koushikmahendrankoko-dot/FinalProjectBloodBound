/* ═══════════════════════════════════════════════════════════════
   BLOODBOUND — progression.js (v3)
   Chapter transitions, save data sync, checkpoint/respawn,
   chapter completion rewards. Achievement hooks added.
═══════════════════════════════════════════════════════════════ */
...
class ProgressionManager {
  ...
  completeChapter(chapterNum) {
    // Track no-death run per chapter
    const deathsThisChapter = (this._chapterDeaths || 0);
    if (chapterNum === 1 && deathsThisChapter === 0) {
      if (window.Achievements) window.Achievements.unlock('no_death_ch1');
    }
    this._chapterDeaths = 0;
    ...
    if (window.Achievements) {
      const achMap = {1:'ch1_clear',2:'ch2_clear',3:'ch3_clear',4:'ch4_clear',5:'ch5_clear'};
      if (achMap[chapterNum]) window.Achievements.unlock(achMap[chapterNum]);
      // All abilities check
      if (this.saveData.unlockedAbilities.length >= 7) window.Achievements.unlock('all_abilities');
    }

    // Story flag achievements
    if (chapterNum === 3) {
      if (window.Achievements) {
        if (this.saveData.storyFlags.sparedLordVael === true)  window.Achievements.unlock('spare_vael');
        if (this.saveData.storyFlags.sparedLordVael === false) window.Achievements.unlock('kill_vael');
      }
    }
  ...
  _getChapterRewards(chapterNum) {
    ...
    return r[chapterNum] || { bloodCoins:0, labels:[] };
  }

  recordDeath() {
    this.saveData.stats.deaths = (this.saveData.stats.deaths||0)+1;
    this._chapterDeaths = (this._chapterDeaths||0)+1;

    // Death achievements
    if (window.Achievements) {
      window.Achievements.unlock('first_death');
      if (this.saveData.stats.deaths >= 10) window.Achievements.unlock('deaths_10');
    }
    this.save();
  }

  getRespawnHp() { return Math.round(this.saveData.maxHp * 0.5); }

  setFlag(key, value) { this.saveData.storyFlags[key]=value; this.save(); }
  getFlag(key)        { return this.saveData.storyFlags[key]; }
  ...
  activateObelisk(obeliskId) {
    ...
    this.save();
    const count = this.saveData.storyFlags.obelisksActivated.length;
    if (count >= 4 && window.Achievements) window.Achievements.unlock('all_obelisks');
    return count;
  }

  allObelisksActivated() { return this.saveData.storyFlags.obelisksActivated.length >= 4; }
  hasSeenDialogue(key)   { return this.saveData.storyFlags.dialogueSeen.includes(key); }
  markDialogueSeen(key)  { if (!this.saveData.storyFlags.dialogueSeen.includes(key)) this.saveData.storyFlags.dialogueSeen.push(key); }
  ...
  resetProgress() {
    if (window.BB && !this.isGuest) { window.BB.resetSaveData(); this.saveData=window.BB.getSaveData(); }
    else { this.saveData=this._fallbackSave(); }
    return this.saveData;
  }

  hasPurchased(itemId) { return this.saveData.purchases.includes(itemId); }
}
